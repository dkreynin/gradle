...
The following code quality plugins each declare a configuration with the same name as the plugin:
...
[[project_getconvention_deprecation]]
=== Deprecated `Project.getConvention()`

Gradle plugins often need to declare configuration with default values that can be read and overridden by build scripts and other plugins.
It used to be a common pattern in plugins to use `Project.getConvention()` to implement such feature.
Now, Gradle has more refined functionality that supersedes conventions:
 - Default values for configuring a custom task: use the Property API.
 - Set up conventional values for all task of the same type: declare the values in a `configureEach()` statement.
 - Declare DSL for a plugin: use extensions.

`Project.getConvention()` is now deprecated and scheduled for removal in Gradle 8.0.
